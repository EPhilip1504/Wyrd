...
export function Form(): React.JSX.Element {
  ...
  const [inputValue, setInputValue] = useState(""); // State for password input

  const onSubmit = async (formData: SignupForm) => {
    try {
      const response = await axios.post(
        REGISTER_URL,
        formData, // Send formData as JSON in the request body
        {
          headers: {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          const errorMessage = error.response.data.error || "An error occurred"; // Adjust key as needed
          const errorType = errorMessage.split(":")[0];
          const startIndex = errorMessage.indexOf('("') + 2;
          const endIndex = errorMessage.lastIndexOf('")');
          if (errorType === "Username") {
            setUsernameError(errorMessage.slice(startIndex, endIndex));
          } else {
            setEmailError(errorMessage.slice(startIndex, endIndex));
          }
        } else if (error.request) {
          setEmailError(
            "Error: No response received from server. Please try again.",
          );
        } else {
  ...
  const handleEmailChange = (event) => {
    ...
    register("email").onChange(event); // Call react-hook-form's `onChange`
  };

  const handleUsernameChange = (event) => {
    setUsernameError(""); // Clear username error
    register("username").onChange(event); // Call react-hook-form's `onChange`
  };
  ...
  return (
    <div>
      <div className="PageContent_pageContent__rDAer layout_container__uRdtW PageContent_centered__XD3ac">
        <div className="Container_container__pBLEY Container_md__dbYCi">
          <div className="page_twoColsWrapper__waRWF">
            <div className="page_controlsWrapper__o44_M">
              <h1 className="page_header__aMEpY">{action}</h1>
              <div className="page_socialButtonsWrapper__xOb3F">
                {" "}
                {/* Add a wrapping parent element */}
                <form
                  className="SocialButton_form__9yToE"
                ...
                >
                  <input
                    ...
                    value="u6kA6til4majs0slTA_9HFjXefF7OB3Ik7VPD_KgrNrqXul6IWGZlbC8BJ8BA8XhPGgZR9FZie8RjSlR_aJnfA"
                  />
                  <button
                    type="submit"
                    className="SocialButton_socialButton__S_le7"
                ...
                <p className="page_orText__GB5K8">or</p>
              </div>

              <form
                className="page_form__qXy18"
                action=""
                onSubmit={handleSubmit(onSubmit)}
              >
                <div className="Input_outerWrapper__0nCNU">
                  <input
                ...
                <div
                  className="Submit_inputWrapper__z2DLF Submit_submitWrapper__CDoSb"
                  tabIndex={0}
                  role="button"
                ...
                >
                  <input
                    type="submit"
                    className="Submit_submit__Qst6_"
                    value="Create account"
                    id="submit_button"
                  />
                  <span className="Submit_glow__0gmKP" />
                </div>
                <p className="page_terms__ivyn3" id="terms">
                  By continuing you agree to our&nbsp;
                  <a href="https://www.raycast.com/terms-of-service">
                    Terms&nbsp;of&nbsp;Service
                  </a>
                </p>
                <a
                  className="page_otherOptionLink__L1EQt"
                  href="/users/sign_in"
                  onClick={() => {
                    const email = document.getElementById("mail");
                    const name = document.getElementById("nameelement");
                    const terms = document.getElementById("terms");
                    if (email !== null && name !== null && terms !== null) {
                      email.remove();
                      name.remove();
                      terms.remove();
                    }
